...
def read_all() -> dict:
    """
    Read all Italian text files and apply apostrophe conversion.

    Loads all cantos from the Divine Comedy using chars.read_all(),
    then converts each line to normalize apostrophe usage (distinguishing
    elision apostrophes from quotation marks).

    Returns:
        dict: Mapping from cantica name to list of cantos, where each canto
              is a list of lines with apostrophes converted.
    """
    import chars

    # Load raw Italian text from it/ directory
    script_dir = Path(__file__).parent
    data = chars.read_all(script_dir.parent / "it")

    # Apply apostrophe conversion to each line in place
    for cantica in data.values():
        for canto in cantica:
            for i in range(len(canto)):
                canto[i] = convert_apostrophe(canto[i])
...
def main():
    ...
    # Read all Italian text from it/ directory
    data = read_all()

    # Process each cantica and canto
    script_dir = Path(__file__).parent
    for cantica, cantos in data.items():
        # Create output directory for this cantica
        (script_dir / cantica).mkdir(exist_ok=True)
        for i, canto in enumerate(cantos, 1):
            txt = script_dir / cantica / f"{i:02d}.txt"
            with open(txt, "w", encoding="utf-8") as f:
                for line in canto:
                    # Tokenize and filter to only alphabetic tokens
                    tokens = [token for token in tokenize(line) if common.has_alpha(token)]
                    # Write pipe-separated tokens
                    print(line, *tokens, sep="|", file=f)
            print(f"Wrote: {cantica}/{i:02d}.txt")
    return 0
